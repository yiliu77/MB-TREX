...
def get_affine_params(num_nets, in_features, out_features):
    w = truncated_normal(
        size=(num_nets, in_features, out_features),
        std=1.0 / (2.0 * np.sqrt(in_features)))
    w = nn.Parameter(w)

    b = nn.Parameter(
        torch.zeros(num_nets, 1, out_features, dtype=torch.float32))
...
#             model_config2["input_dim"] = [model_config["architecture"][i]["new_input_dim"]]
#             return model_config1, model_config2
#     return None, model_config2
#
#
# ACTIVATION_DICT = {'relu': nn.ReLU(), 'leaky_relu': nn.LeakyReLU(),'none': lambda x: x}
#
#
...
#         self.cnn_layers, self.output_dim = Model._init_cnn_layers(input_dim, cnn_config, self.hidden_act)
#         self.linear_layers, self.output_dim = Model._init_linear_layers(self.output_dim, linear_config, self.hidden_act)
#         self.left_layers, self.right_layers = Model._init_split_layers(self.output_dim, split_config, self.hidden_act)
#
...
#
#     @staticmethod
#     def _init_cnn_layers(input_dim, cnn_config, hidden_activation):
...
#
#     @staticmethod
#     def _init_linear_layers(input_dim, linear_config, hidden_activation):
...
#
#     @staticmethod
#     def _init_split_layers(input_dim, split_config, hidden_activation):
